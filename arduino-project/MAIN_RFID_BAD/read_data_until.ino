...
boolean read_until_ESP(const char keyword1[], int key_size, int timeout_val, byte mode)
{
 ...
 //FILL UP THE BUFFER
  for(byte i=0; i<key_size; i++){//we only need a buffer as long as the keyword
    //timing control
    while(!EspSerial.available()){//wait until a new byte is sent down from the ESP - good way to keep in lock-step with the serial port
    ...
    data_in[i]=EspSerial.read();// save the byte to the buffer 'data_in[]
...
//THE BUFFER IS FULL, SO START ROLLING NEW DATA IN AND OLD DATA OUT
  while(1){//stay in here until the keyword found or a timeout occurs
    ...
    //timing control
    while(!EspSerial.available()){// same thing as done in the buffer
    ...
    data_in[key_size-1]=EspSerial.read();//save the new data in the last position in the buffer
